...
The `Functional` is a global instance having functional methods which is useful with a pipeline operator.
...
|         Name          |                                                          Outline                                                          |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| filter(func)          | Filter by the element returned by `func`.                                                                                 |
| map(func)             | Map by the element returned by `func`.                                                                                    |
| flatMap(func)         | Map by the element returned by `func`, then flatten the all result.                                                       |
| take(n)               | Take `n` elements from the beginning.                                                                                     |
| takeWhile(func)       | Take an element while returning true by `func`.                                                                           |
| drop(n)               | Skip `n` elements from the beginning.                                                                                     |
| dropWhile(func)       | Skip an element while returning true by `func`.                                                                           |
| each(func)            | Applying `func` for each.                                                                                                 |
| reduce(func, initer)  | Reduce by the result of `func`. `initer` is the initial value if necessary.                                               |
| sort(func)            | Sort elements by comparing with the result of `func`. Note that this method will fetch all at once to sort.               |
| println/println(msg)  | Print the value and pass it to the next. This is used to see inside the pipeline. `msg` can be lead message of output.    |

The following methods will aggregate a sequence.
By one of those, the pipeline will be terminated and a value will be aggregated.
