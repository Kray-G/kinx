...
\\url\[https://www.chartjs.org/\]{Chart.js}\\footnote{https://www.chartjs.org/} のデータそのものを `options` フィールドに記載します。
...
上記は以下のように記載されます。
尚、ここでは紙面の都合上、省略しますが、`options` は Chart.js のオプションです。

    ```chart
    {
        float: { right: true },
        width: 800, height: 500, scale: 0.4, caption: "Line Chart Example",
        options: {
            type: "line",
            ...(省略)
        }
