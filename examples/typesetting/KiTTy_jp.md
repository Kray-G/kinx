...
外部リンクは直接 URL 文字列を記載することで自動的に判別されます。
例えば、 https://github.com/Kray-G/kinx と記載することで自動的に外部リンクが生成されます。
...
注意点として、見出しには KiTTy コマンドなどのコマンドを使用することはできません。
したがって、見出し文字列には各種コマンドを利用しないようにしてください。
