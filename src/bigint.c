...
bigint* bigint_from_int(bigint *dst, int src){
    ...
    #if BIGINT_ENDIAN == LITTLE_ENDIAN
    memcpy(dst->words, &x, sizeof(x));
    #elif BIGINT_ENDIAN == BIG_ENDIAN
    for (int i = 0; i < n; ++i) {
        dst->words[i] = x & BIGINT_WORD_MASK;
        x >>= BIGINT_WORD_BITS;
    }
    #else
...
bigint* bigint_from_int64(bigint *dst, int64_t src){
    uint64_t x = BIGINT_INT64_ABS(src);
    int n = BIGINT_MAX(1, sizeof(x)/sizeof(bigint_word));
    bigint_reserve(dst, n);
    bigint_raw_zero(dst->words, 0, n);
    #if BIGINT_ENDIAN == LITTLE_ENDIAN
