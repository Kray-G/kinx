...
int test(void)
{
    ...
    int start = kv_size(code);
        ...
        // und & dbl -> int(same)
            ...
            kv_push(kx_code_t, code, ((kx_code_t){ .op = KX_CHKVAL, .file = const_str(ctx, __FILE__), .line = __LINE__, .value1.i = KX_INT_T, .value2.i = 10 }));
        // dbl & und -> int(same)
        ...
        // big & big -> big
            ...
            kv_push(kx_code_t, code, ((kx_code_t){ .op = KX_XOR }));
            kv_push(kx_code_t, code, ((kx_code_t){ .op = KX_CHKVAL, .file = const_str(ctx, __FILE__), .line = __LINE__, .value1.i = KX_BIG_T, .value2.s = "27670116110564327424" }));
